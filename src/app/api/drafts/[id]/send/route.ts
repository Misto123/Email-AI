...
export async function POST(_request: Request, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    const { data: draft, error } = await supabaseAdmin.from("drafts").select("id,draft_body,status,emails(from_email,from_name,subject,message_id,thread_id,in_reply_to,references),mailboxes(email,encrypted_password)").eq("id", id).single();
    ...
    const emailValue = draft.emails as unknown as { from_email: string; from_name: string | null; subject: string; message_id: string; thread_id: string | null; in_reply_to: string | null; references: string | null } | Array<{ from_email: string; from_name: string | null; subject: string; message_id: string; thread_id: string | null; in_reply_to: string | null; references: string | null }>;
    ...
    const { error: updateError } = await supabaseAdmin.from("drafts").update({ status: "sent", updated_at: now, sent_at: now }).eq("id", id);
